...
C線 final acceptance checkpoint（2026-08-12）：Access session 下的期限頁唯讀載入成功，標題為「重要期限與多通道警告」且無紅色 API／載入錯誤；使用者已準備一筆正式 `OPEN` 期限並完成真實電腦授權／啟用。使用者提供的畫面顯示電腦確實收到測試通知，App 顯示 `READY`／`ACTIVE`、最近成功時間、成功 1／失敗 0；D1 唯讀聚合確認訂閱成功 1／錯誤 0、`WEB_PUSH=READY`、delivery `SENT` 1。下一步只驗收手機，再做逐裝置狀態讀回及獨立停用；`AT-PUSH-01` 維持 `IN_PROGRESS`。

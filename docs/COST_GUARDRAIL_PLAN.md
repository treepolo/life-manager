...
| Resend | Free 100 emails/day、3,000/month、10 req/s（官方目前文件）；429、daily／monthly quota error 或 provider pause 是停止訊號，不能自動升級。未取得本帳戶 plan／remaining／reset 時不以 Free baseline 放行。 | response `ratelimit-*`、daily／monthly quota headers、delivery attempts、429／quota errors；日／月 window 依 Resend 回應與 Usage。 | `resource + provider_team + window + threshold` 去重；同一 notification operation 不因告警重試而多寄。 | 停非必要測試信與低優先通知，保留站內／Push；按 rate header backoff，不以重試突破 quota。 | circuit breaker 封鎖 Resend send，保存去敏錯誤；不能用「卡片」或自動 plan upgrade 解決。 | 下一 provider reset／Usage 確認後，管理者解除 breaker；先發一封明確 test，核對 delivery／idempotency，再恢復。 | Notifications／Resend owner；Resend 帳戶管理者控制 plan。 |
| YouTube Data／Analytics quota | Data API default 10,000 units/day；method cost 以官方表固定答案，所有 request 至少 1；在沒有 account current usage 時可用官方 baseline 建立 `LOCAL_CONSERVATIVE`，但只能標 `ESTIMATED`／`providerInvoiceTruth=false`，並採 hard-reject-only 75%／85% reserve。Analytics quota／帳戶 billing 未以 Data API 數字代替，unknown 只阻擋 Analytics request。達 quota 是 provider error／拒絕，不以 request 次數估算。 | Google API Console quota、response error、method cost、Pacific day reset；保存 project／API、method、units、period、source、quality。 | method cost exact 才能計；Analytics allowance 或 meter unknown 時不發起 metrics request，但不阻擋 Data API 或不相關 read。 | Data API local baseline 使用 hard-reject-only 的 75% degrade／85% internal stop；Analytics unknown 直接 fail-closed。 | quota error、metric/API timeout 或 method cost unknown 時對應 operation fail-closed，不發起下一個外部 request；不申請／切換付費 quota。 | Pacific reset 後由 Console／API 重新確認，管理者解除 provider breaker，再用單次 synthetic／本人批准同步驗證。 | YouTube provider owner；Google project 管理者控制 quota／billing。 |
...
- 每個 provider budget 保存 operation class、cost unit、period、remaining、quality、reason；只有 contract 明確允許且有官方 included baseline 的資源才可用 `LOCAL_CONSERVATIVE` 放行，UI／API 必須標 `ESTIMATED`／`NOT_INVOICE_TRUTH`；exact allowance／measurement unknown 的 operation 不得用估算放行。
...
- 已實作：`provider-sync.ts` 的 scheduled／manual shared guard、YouTube Data／Analytics partial gate、Instagram／Resend request gate、D1 rows／storage persistence admission、`COST_GUARDRAIL_*` API errors、狀態 API／Integrations UI 與 expiry override audit。YouTube Data／D1 official baseline 為 local conservative only；Instagram／Resend unknown 仍各自 fail-closed。
...
- 本階段部署順序：先本機固定答案與掃描，再確認 remote pending 僅 0011／0012，套 staging migrations，使用 `wrangler deploy --config wrangler.toml --env staging --keep-vars`，再做不呼叫真實 provider 的 staging API／UI／gate smoke；不部署 production。production 前必須有 `SETUP-010` 證據與 authoritative account reconciliation。
...
- migration：`0011`／`0012` 為新增 append-only、forward-only migration；local reapply／sentinel test 已證明 Wrangler migration tracking 可重跑且不破壞資料。staging 只允許依 remote list 套用 0011／0012；rollback 只允許依 migration backup／新 forward migration，由單一帳戶設定線決定，不能刪除 append-only evidence。

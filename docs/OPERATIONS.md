...
- 電腦真人驗收已完成：使用者畫面顯示測試通知已收到、channel `READY`、裝置 `ACTIVE`、最近成功時間及成功 1／失敗 0；D1 唯讀聚合顯示訂閱成功 1／錯誤 0、`WEB_PUSH=READY`、delivery `SENT` 1／錯誤 0。C 下一步依 SETUP-006 只要求真實手機完成授權／訂閱／收件，再做獨立停用核對。

...
| DDL | `src/modules/deadlines`, `src/modules/notifications`, `src/integrations/resend`, `src/worker/scheduled`；`0005` | W-8固定答案、範本級別、子任務、dedupe、密文雙裝置Push契約；`tests/unit/resend.test.ts`、`tests/worker/resend-d1.test.ts`驗證Resend測試信標示、provider錯誤映射、delivery log、重試與去重；遠端D1已核對加密收件地址、Email `READY`、1筆正式`OPEN`期限、真實`SENT` delivery及provider message ID非空；使用者確認在垃圾郵件收到測試信；C線Push設定與真人裝置驗收仍未完成 | `DDL-008`／`SETUP-006`／`AT-PUSH-01`為`IN_PROGRESS`，等待VAPID staging、client build、Access授權、手機／電腦實收、獨立停用及共用scheduler每裝置狀態回寫；`DDL-009`／`SETUP-005`／`AT-MAIL-01`為`VERIFIED`；共用通道摘要缺陷已移交N線；其餘`VERIFIED` |

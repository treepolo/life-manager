...
| 導入Firstrade帳務資料 | INV-002 | `src/integrations/firstrade-csv/importer.ts`, `src/integrations/firstrade-csv/service.ts`, `src/app/pages/InvestmentImportPanel.tsx`, `src/worker/api/index.ts`；`0003_finance_investments.sql` | AT-INV-02～04已有`tests/unit/firstrade.test.ts`／`tests/worker/api-d1.test.ts`固定答案；遮蔽真實樣本486列／0 parse errors／UTF-8逗號／Amount USD 17.81；D1首次485新增+1同檔重複、重跑0新增+486重複、未知Other 27列原始證據保留；AT-INV-05尚待App預覽與官方畫面核對 |
...
| FIN、INV | `src/modules/finance`, `src/core/money`, `src/integrations/firstrade-csv`, `InvestmentImportPanel.tsx`, `FinancePage.tsx`；`0003` | `finance.test.ts`, `tests/unit/firstrade.test.ts`, `tests/worker/api-d1.test.ts`：預覽／七類活動／未知類型／D1去重與原始證據契約；遮蔽真實樣本的row count、Amount合計、活動類型與重跑結果已取得，仍待App預覽、官方畫面抽樣及正式匯入前備份 | `INV-002`／`SETUP-007`為`AWAITING_USER_SETUP`；其餘`VERIFIED` |

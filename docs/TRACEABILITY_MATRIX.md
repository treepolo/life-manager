...
| `REM-NAV-001` | `AT-REM-NAV-001`～`006` | `src/app/layouts/AppShell.tsx`、`src/styles.css`（navigation／safe-area／fixed surface）、`tests/unit/app-shell-navigation.test.tsx`、`tests/e2e/app.spec.ts` | 不新增 migration；不修改既有 migration | `IMPLEMENTED_UNVERIFIED` | desktop九route；手機四主要入口＋「更多」五secondary與系統sync／PWA status；1366／1280／768／390／320 local browser、pre-keyboard E2E／unit keyboard、back／reload／offline／401 simulation、geometry與無假資料 evidence；post-keyboard E2E受Wrangler startup blocker；Access／staging／真人手機未驗收 | 後續唯一 integrator；real Access／phone human checkpoint |
...
## RETROFIT-W2A-MOBILE-NAV execution evidence（2026-08-14）

本節只記錄`RETROFIT-W2A-MOBILE-NAV`的client navigation／responsive shell scope；current status仍唯一由`docs/IMPLEMENTATION_STATUS.md`宣告。branch為`codex/accept-external-integrations`；起點`fe6448575fa935425cdec3dfed574811ff49a824`符合派工要求，未建立外部worktree、未switch／reset／merge、未讀取secret、未執行OAuth／provider／billing／remote D1／deploy，且未新增或修改migration。

- 實作：`AppShell.tsx`保留desktop九個正式`NavLink`；mobile固定今日／總覽／任務／財務四個主要入口，將社群／重要期限／指標／事件／外部連線／資料管理放入可發現的「更多」，並重用既有sync action與DOM觀測到的`PwaUpdate`固定提示狀態。`aria-expanded`／`aria-controls`、Enter／Tab／Escape、outside close、route-change close、focus return與active route均有實作；未新增route或router抽象。
- 樣式：`src/styles.css`只新增navigation focus／menu、44px touch target、safe-area bottom inset、mobile fixed sync／nav／more layout與update-banner避讓；320px不以隱藏溢位替代可用性。
- Fixed-answer／unit：`tests/unit/app-shell-navigation.test.tsx`核對九 desktop links、mobile route inventory、secondary/system discovery、PWA status、`aria-expanded`、Escape focus return與route click close。
- Browser／visual：加入Enter／Tab斷言前，`tests/e2e/app.spec.ts`的`REM-NAV-001`在fresh local D1對desktop、tablet、mobile-390、mobile-320各自通過；desktop九route與1280／1366 screenshots、mobile正式path到達期限／指標／外部連線／資料、Escape／active／back／deep reload、offline navigation、401 session simulation、無示範資料、scrollWidth、safe fixed surfaces與44px targets有證據。加入Enter／Tab斷言後的同一targeted command已執行兩次隔離runtime retry，均在Wrangler啟動階段中斷，未進入Playwright assertion；keyboard部分目前由unit／source evidence覆蓋，等待穩定runtime重跑。這是local synthetic evidence，不是Access／staging／真人scenario。
- Acceptance mapping：`AT-REM-NAV-001`、`003`、`004`與`006`具local PASS證據；`AT-REM-NAV-002`為unit＋pre-keyboard E2E partial，`AT-REM-NAV-005`只有local 401 simulation，real Access/server unauthorized route未執行；整體因此為`IMPLEMENTED_UNVERIFIED`，後續由唯一 integrator安排穩定runtime、Access與真人手機／production gate。

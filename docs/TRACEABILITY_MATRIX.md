...
| DDL | `src/modules/deadlines`, `src/modules/notifications`, `src/integrations/resend`, `src/worker/scheduled`；`0005` | W-8固定答案、範本級別、子任務、dedupe、密文雙裝置Push契約；`AT-PUSH-01`自動契約已通過 | `DDL-008`／`SETUP-006`／`AT-PUSH-01`於2026-08-11由C線轉`IN_PROGRESS`，等待VAPID staging設定、Access授權、真實手機／電腦通知與獨立停用；另發現每裝置成功／錯誤狀態未由共用scheduler回寫，依工作分線規則移交，不在C線修改共用檔案；`DDL-009`仍等待真實郵件；其餘`VERIFIED` |

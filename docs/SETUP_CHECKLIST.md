...
目前狀態：`VERIFIED`。`INV-002`的通用 importer、預覽、驗證、活動正規化、未知類型原始證據與 D1 去重契約已有自動測試；遮蔽真實副本已完成staging App預覽與Firstrade官方唯讀紀錄核對，正式staging D1 SQL／App完整JSON備份也已完成並核對。正式匯入已於共用staging版本`db41ff0c-7864-43d2-9a98-54000cebfa92`完成，未登入health為302、遠端migration無待套用。未遮蔽原檔、帳號、姓名、地址與帳密不得交給 Codex，也不得進入 Git、log、snapshot 或文件。

遮蔽樣本證據：staging App預覽顯示486列、UTF-8逗號、0 parse errors；正式匯入結果顯示486／486／0／0；D1去識別合計為USD 17.81（1781 minor units），活動BUY255／SELL182／DIVIDEND7／INTEREST15／UNCLASSIFIED27，`Other` 27列保留原始證據，重跑0新增／486重複。Firstrade官方帳戶紀錄完整日期範圍顯示總數486，唯讀篩選確認2025-04-16兩筆MSTU BUY的數量1.00與金額-5.71均與遮蔽樣本一致。CSV沒有Currency欄，USD目前只是mapping profile的default設定，不能冒充來源欄位。

本輪正式驗收已完成：未套用migration、不讀取Secret值；正式匯入batch `019ffa7d-2483-7396-87a3-fdd7c8a5ba6e`於`2026-08-13T09:42:02.922Z`完成，結果486／486／0／0，並完成Firstrade官方來源兩筆人工核對。不要貼任何CSV內容；未遮蔽原檔、帳號、姓名、地址與帳密仍不得進入Codex、Git、log、snapshot或文件。
...
- [x] 含修正版本的staging部署完成：先前B線版本`7d7171c5-7be9-49c8-a28b-e7e0b2dfbe18`承接100%流量；最終共用staging版本`db41ff0c-7864-43d2-9a98-54000cebfa92`承接100%流量，未登入health回302，已登入`/finance`載入正常，遠端migration無待套用。
- [x] 在最終共用staging執行正式匯入，結果486／486／0／0；D1 Amount合計USD 17.81，並完成Firstrade官方畫面兩筆來源交易核對以完成AT-INV-05。
- [x] 正式匯入前先做D1／JSON備份；SQL與App JSON檔案均在被Git忽略的`backups/`，已核對bytes、SHA-256 sidecar與JSON內部checksum，未提交內容。

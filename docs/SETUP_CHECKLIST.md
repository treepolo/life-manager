...
目前狀態：`AWAITING_USER_SETUP`。`INV-002`的通用 importer、預覽、驗證、活動正規化、未知類型原始證據與 D1 去重契約已有自動測試；AT-INV-05 只等待官方實際 CSV 的遮蔽樣本及畫面核對。未遮蔽原檔、帳號、姓名、地址與帳密不得交給 Codex，也不得進入 Git、log、snapshot 或文件。

第一個且唯一的下一個人工操作：請先在 Firstrade 官方介面匯出帳務／歷史 CSV。這一步只完成官方匯出，不要把檔案內容貼到聊天，也不要同時進行後續遮蔽、上傳或畫面核對；完成後再由本線逐步要求下一項操作。

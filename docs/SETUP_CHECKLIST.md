...
目前狀態：`AWAITING_USER_SETUP`（2026-08-12，D線 `codex/accept-resend`；帳號、兩個Secret與收件設定已完成，等待下一個真人資料操作）。

本次真實驗收仍精確缺少：

- 尚未取得本人實際收到測試信，以及其delivery log `SENT`／provider message ID對應證據。
- staging目前沒有未刪除／未封存的正式`OPEN`期限；因此「測試發送」按鈕依既有UI條件停用。
...
- [ ] 在App執行測試信；信件主旨／本文必須明確包含期限名稱、重要級別、App連結與「這是使用者觸發的測試」，且不得建立假期限；必須先有一筆正式`OPEN`期限。

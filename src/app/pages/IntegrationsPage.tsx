...
export function IntegrationsPage() {
  ...
  const sync = useMutation({ mutationFn: (connection: Connection) => apiPost(`/api/v1/integrations/${connection.id}/sync`, { operationId: uuidv7(), from: `${new Date().getUTCFullYear()}-01-01`, to: new Date().toISOString().slice(0, 10) }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["integrations"] }), onError: setActionError });
  ...
  return <div className="page"><PageHeader eyebrow="ADAPTER / AUTHORIZATION" title="外部連線與同步證據" description="OAuth token 只存伺服器密文。未授權時不會用假資料冒充同步成功。" />
    ...
    <div className="split-layout">{(["youtube", "instagram"] as const).map((provider, index) => { const connection = byProvider.get(provider); return <Panel key={provider} title={provider === "youtube" ? "YouTube" : "Instagram"} index={`0${index + 1}`} tone={connection?.status === "CONNECTED" ? "accent" : "default"}><div className="integration-card"><div><StatusMark tone={connection?.status === "CONNECTED" ? "good" : connection ? "danger" : "pending"}>{connection?.status ?? "等待設定"}</StatusMark><h3>{connection?.display_name ?? (provider === "youtube" ? "尚未授權頻道" : "尚未授權專業帳號")}</h3><p>{provider === "youtube" ? "唯讀 YouTube Data API 與 Analytics；不要求金額權限。" : "Instagram Login 專業帳號基本資料與 insights 唯讀同步。"}</p>{connection ? <dl className="definition-grid"><div><dt>最近嘗試</dt><dd>{connection.last_attempt_at ? new Date(connection.last_attempt_at).toLocaleString("zh-TW") : "尚無"}</dd></div><div><dt>最近成功</dt><dd>{connection.last_success_at ? new Date(connection.last_success_at).toLocaleString("zh-TW") : "尚無"}</dd></div><div><dt>下次排程</dt><dd>{connection.next_run_at ? `${new Date(connection.next_run_at).toLocaleString("zh-TW")} · ${connection.sync_job_status} · 第 ${connection.sync_attempt ?? 0} 次` : "尚未建立"}</dd></div><div><dt>Token 到期</dt><dd>{connection.token_expires_at ? new Date(connection.token_expires_at).toLocaleString("zh-TW") : "來源未回報"}</dd></div><div><dt>來源定義版本</dt><dd>{connection.provider_definition_version}</dd></div><div><dt>最近錯誤</dt><dd>{connection.last_error_code ? `${connection.last_error_code} · ${connection.last_error_message_redacted ?? "無去敏訊息"}` : "無"}</dd></div></dl> : null}</div><div className="row-actions">{!connection || connection.status !== "CONNECTED" ? <button className="button" type="button" onClick={() => authorize.mutate(provider)}>開始正式授權</button> : <><button className="button" type="button" onClick={() => sync.mutate(connection)}>立即同步</button><button className="button button--quiet" type="button" onClick={() => disconnect.mutate(connection)}>撤銷連線</button></>}</div></div></Panel>; })}</div>
